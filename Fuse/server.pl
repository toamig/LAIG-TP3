...
parse_input(valid_moves(Board, Piece), Reply):-
	valid_moves_piece(Board, Piece, ListOfMoves),
	list_to_json(ListOfMoves,Reply).

parse_input(score(Board) ,Reply):-
	value(Board, [IceScore,BlackScore]),
	list_to_json([IceScore,BlackScore],Reply).

parse_input(play_bot(Player,Level,Board), Reply):-
	choose_move(Board, Player, Level, Move),
	list_to_json(Direction,DirectionString),
	matrix_to_json(Move,Reply).